...
const getResolveConfigPath = (pathToConfigs) => {
  ...
  try {
    ...
    process.env.DEBUG &&
      console.log(
        '\n[@stackr23/config-eslint] Info: try to load resolve config from:\n',
        resolvedPath,
      )
    ...
    if (e.code === 'MODULE_NOT_FOUND') {
      process.env.DEBUG &&
        console.log(
          ...
          '\n[@stackr23/config-eslint] webpack.config.resolve.js not found - using \'@stackr23/config-eslint/utils/webpack.config.resolve.js\'\n',
        )
    }
    else {
      throw e
  ...
  process.env.DEBUG &&
    console.log(
      '\n[@stackr23/config-eslint] Info: resolve config found under:\n',
      resolvedPath,
      '\n',
    )
